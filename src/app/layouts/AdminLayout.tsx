...
export function AdminLayout() {
  ...
  return (
    <div className="flex flex-col min-h-screen w-full bg-[#FDFBFB] dark:bg-[#0F0F0F] text-neutral-900 dark:text-neutral-100 font-sans md:max-w-none md:border-x-0 md:border-t-0 mx-auto max-w-md shadow-2xl relative border-x-4 border-t-4 border-[#3A0310] transition-all duration-300">
      <ScrollToTop />
      
      {/* Header */}
      <header className="bg-white/80 dark:bg-[#0F0F0F]/80 backdrop-blur-md border-b border-[#3A0310]/20 px-6 py-4 flex items-center justify-between shadow-md z-10 sticky top-0 w-full">
      ...
      <nav className="bg-white dark:bg-gray-950 border-t border-[#3A0310]/20 fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md pb-safe-area flex md:hidden justify-around items-center h-16 shadow-[0_-4px_15px_-1px_rgba(0,0,0,0.8)] z-20 border-x-4 border-[#3A0310] border-b-4 border-b-[#3A0310]">
        {navItems.map((item) => (
          <NavLink
            key={item.to}
            to={item.to}
