      ...
      <AnimatePresence>
        {resetResult && (
          <motion.div
            ...
            className="bg-gradient-to-br from-[#3A0310] to-[#5A0520] rounded-2xl p-6 text-white force-white shadow-2xl border border-[#E8B4B8]/20"
          >
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <CheckCircle2 className="w-5 h-5 text-green-400" />
                <h3 className="font-black uppercase tracking-widest text-xs force-white">Senha Gerada com Sucesso</h3>
              </div>
              <button onClick={() => setResetResult(null)} className="p-1 rounded-lg hover:bg-white/10 transition-colors">
                <X className="w-4 h-4 force-white" />
              </button>
            </div>

            <div className="bg-white/10 rounded-xl p-4 mb-4">
              <p className="text-[10px] font-bold uppercase tracking-widest text-white/60 mb-1 force-white">Utilizador</p>
              <p className="font-black text-sm force-white">{resetResult.name}</p>
              <p className="text-xs text-white/60 force-white">{resetResult.email}</p>
            </div>

            <div className="bg-white/10 rounded-xl p-4 mb-4">
              <p className="text-[10px] font-bold uppercase tracking-widest text-white/60 mb-2 force-white">Nova Senha Gerada</p>
              <div className="flex items-center gap-2">
                <code className="flex-1 bg-black/30 rounded-lg px-4 py-3 font-mono text-lg font-black tracking-wider text-[#E8B4B8] force-gold select-all">
                ...
                >
                  {copiedId === 'modal' ? <Check className="w-5 h-5 text-green-400" /> : <Copy className="w-5 h-5 force-white" />}
                </button>
              </div>
            </div>

            <p className="text-[10px] text-white/50 text-center force-white">Copie a senha e envie ao utilizador por email ou mensagem.</p>
          </motion.div>
        )}
      </AnimatePresence>
            ...
            >
              <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl font-black uppercase text-[10px] tracking-widest bg-gradient-to-r from-[#3A0310] to-[#E8B4B8] text-white force-white hover:opacity-90 transition-opacity disabled:opacity-50 shadow-md shrink-0 w-full sm:w-auto"
                >
                  {processingId === req.id ? (
